...
const register = async (req, res) => {
  try {
    ...
    req.logger.grave(`Error registering user: ${error.message}`);
...
const current = async (req, res) => {
  ...
  if (!cookie) {
    req.logger.warn("Unauthorized access attempt to current user");
    return res.status(401).send({ status: "error", error: "Unauthorized" });
  }
  const user = jwt.verify(cookie, "tokenSecretJWT");
  if (user) {
    req.logger.info("Current user retrieved");
